...
function algorithmsRuntime() {
  ...
  function read(file, maxBytes = 2_000_000) {
    try {
  ...
  function safeScan(input, options = {}) {
    if (!input || !input.root || !fs.existsSync(input.root)) return [];
    return scan(input.root, { maxFiles: input.maxFiles || options.maxFiles || 10_000, signal: input.signal }).filter((item) => options.binary === true || !item.binary);
  }
  function escapeRegex(value) {
    return String(value).replace(/[.*+?^$(){}|[\]\\]/g, "\\$&");
  }
  function moduleImports(source, extension) {
    const result = [];
    ...
    for (const expression of expressions) {
      let match;
      while ((match = expression.exec(source))) result.push(match[1] || match[2] || match[3]);
    }
    return result;
  }
  function exportedSymbols(source) {
    ...
    for (const expression of expressions) {
      let match;
      while ((match = expression.exec(source))) result.add(match[1]);
    }
    const objectExpression = /module\.exports\s*=\s*\{([^}]+)\}/g;
    let objectMatch;
    while ((objectMatch = objectExpression.exec(source))) {
      objectMatch[1].split(",").map((item) => item.split(":")[0].trim()).filter(Boolean).forEach((name) => result.add(name));
    }
    return Array.from(result);
  }
  function placeholders(value) {
    return Array.from(String(value).matchAll(/\{\{?([A-Za-z0-9_]+)\}?\}|%[sd]|\$\{([A-Za-z0-9_]+)\}/g)).map((match) => match[1] || match[2] || match[0]).sort();
  }
  ...
  function parsePatch(diff) {
    ...
    for (const line of String(diff || "").split(/\r?\n/)) {
      if (line.startsWith("+++ b/")) {
        current = { path: line.slice(6), added: [], removed: [] };
        files.push(current);
      } else if (current && line.startsWith("+") && !line.startsWith("+++")) current.added.push(line.slice(1));
      else if (current && line.startsWith("-") && !line.startsWith("---")) current.removed.push(line.slice(1));
    }
    return files;
  }

  async function repositoryMap(input = {}) {
    const files = safeScan(input);
    const byExt = {}, byLanguage = {}, manifests = [];
    const languages = { ".js":"JavaScript", ".ts":"TypeScript", ".tsx":"TSX", ".py":"Python", ".cs":"C#", ".java":"Java", ".go":"Go", ".rs":"Rust", ".json":"JSON", ".md":"Markdown" };
    let totalBytes = 0;
    for (const item of files) {
      byExt[item.ext || "none"] = (byExt[item.ext || "none"] || 0) + 1;
      const language = languages[item.ext] || "Other";
  ...
  async function contextBuilder(input = {}) {
    const taskWords = words(input.task || ""), active = new Set(input.activeFiles || []), dirty = new Set(input.dirtyFiles || []), budget = input.tokenBudget || 6000, scored = [];
    for (const item of safeScan(input)) {
      const source = read(item.absolute, input.maxFileBytes || 500_000);
      if (!source) continue;
      let score = 0;
      const reasons = [];
      ...
      for (const word of taskWords) {
        ...
        const matches = source.toLowerCase().match(new RegExp(escapeRegex(word), "g")) || [];
        if (matches.length) { score += Math.min(10, matches.length); reasons.push("content:" + word); }
      }
      scored.push({ path:item.path, score, reasons, tokens:tokensRough(source) });
    }
  ...
  async function codeReview(input = {}) {
    const rules = [
      ["eval","high",/\beval\s*\(/g], ["new-function","high",/new\s+Function\s*\(/g], ["empty-catch","medium",/catch\s*\([^)]*\)\s*\{\s*\}/g],
      ["inner-html","high",/\.innerHTML\s*=/g], ["node-integration","critical",/nodeIntegration\s*:\s*true/g], ["context-isolation-off","critical",/contextIsolation\s*:\s*false/g],
      ["sandbox-off","high",/sandbox\s*:\s*false/g], ["web-security-off","critical",/webSecurity\s*:\s*false/g], ["insecure-content","critical",/allowRunningInsecureContent\s*:\s*true/g],
      ["sync-io","low",/\b(?:fs\.)?(?:readFileSync|writeFileSync|readdirSync|statSync)\s*\(/g]
    ];
    const findings = [];
    for (const file of input.files || []) {
      const source = read(file), lines = source.split(/\r?\n/);
      for (const [rule,severity,expression] of rules) {
  ...
  async function testGenerator(input = {}) {
    const results = [];
    for (const file of input.files || []) {
      const source = read(file), symbols = exportedSymbols(source);
      ...
      results.push({ file, framework, symbols, importSuggestion:framework === "pytest" ? "from module import symbol" : "const { symbol } = require('./module')", cases:symbols.flatMap((symbol)=>[
        {symbol,kind:"happy",title:symbol+" geçerli girdide çalışır"}, {symbol,kind:"boundary",title:symbol+" sınır girdisini yönetir"},
        {symbol,kind:"error",title:symbol+" geçersiz girdiyi reddeder"}, {symbol,kind:"security",title:symbol+" zararlı girdiyi sınırlar"}
      ]) });
    }
    return { results };
  }
  ...
  async function dependencyAuditor(input = {}) {
    const root = input.root || process.cwd(), packagePath = path.join(root,"package.json");
    let pkg;
    ...
    const locks = ["package-lock.json","pnpm-lock.yaml","yarn.lock","bun.lock","bun.lockb"].filter((name)=>fs.existsSync(path.join(root,name))), issues = [];
  ...
  async function documentationGenerator(input = {}) {
    ...
    const projectFiles = fs.existsSync(root) ? scan(root,{maxFiles:5000}) : [], has = (name) => projectFiles.some((item)=>item.path===name || item.path.endsWith("/"+name)), scripts = pkg.scripts || {};
    const sections = ["# "+(pkg.name||path.basename(root)),pkg.description||"","## Kurulum","npm install","## Komutlar"];
    for (const [name,command] of Object.entries(scripts)) sections.push("- npm run "+name+": "+command);
    sections.push("## Algılanan Yapı","- Electron: "+Boolean(pkg.dependencies?.electron||pkg.devDependencies?.electron),"- Preload: "+has("preload.js"),"- Test: "+Boolean(scripts.test));
  ...
  async function changelogBuilder(input = {}) {
    ...
    for (const commit of input.commits || []) {
      const message = String(commit.message || ""), lower = message.toLowerCase();
  ...
  async function commitMessageAssistant(input = {}) {
    const diff = input.diff || "", type = /test/.test(diff) ? "test" : /README|docs\//i.test(diff) ? "docs" : /fix|error|bug/i.test(diff) ? "fix" : /package(?:-lock)?\.json/.test(diff) ? "chore" : "feat", scope = input.scope ? "("+input.scope+")" : "";
  ...
  async function promptLibrary(input = {}) {
    ...
    if (action === "add") {
      const body = String(input.body || ""), secrets = scanSecrets(body), item = { id:input.id||crypto.randomUUID(), title:input.title||"Başlıksız", body:secrets.length?"[REDACTED]":body, tags:input.tags||[], version:1, createdAt:new Date().toISOString() };
  ...
  async function fileSummarizer(input = {}) {
    const results = [];
    for (const file of input.files || []) {
      const source = read(file), imports = Array.from(source.matchAll(/(?:require\(["']([^"']+)|from\s+["']([^"']+)|import\s+["']([^"']+))/g)).map((m)=>m[1]||m[2]||m[3]), symbols = Array.from(source.matchAll(/(?:function|class|const|let)\s+(\w+)/g)).map((m)=>m[1]), publicApi = Array.from(source.matchAll(/(?:exports\.(\w+)|export\s+(?:function|class|const)\s+(\w+))/g)).map((m)=>m[1]||m[2]), todos = Array.from(source.matchAll(/\b(TODO|FIXME|HACK)\b[: ]*(.*)/gi)).map((m)=>({tag:m[1],text:m[2].trim()}));
      results.push({file,lines:source.split(/\r?\n/).length,complexity:(source.match(/\b(if|for|while|switch|case|catch)\b/g)||[]).length,imports:Array.from(new Set(imports)),symbols:Array.from(new Set(symbols)).slice(0,200),publicApi:Array.from(new Set(publicApi)),todos});
    }
    return {results};
  }
  async function localSearch(input = {}) {
    if (!input.root) return {matches:[],truncated:false};
    if (String(input.query||"").length>300) throw new Error("pattern uzun");
    if (input.regex&&/(?:\(.+\)){5,}|\*\+|\+\*/.test(input.query)) throw new Error("karmaşık regex");
    const expression = input.regex ? String(input.query) : escapeRegex(input.query||""), regex = new RegExp(expression,input.caseSensitive?"":"i"), matches = [];
    for (const item of safeScan(input)) {
      if (item.binary||item.size>(input.maxFileBytes||1_000_000)) continue;
      if (input.ext?.length&&!input.ext.includes(item.ext)) continue;
      if (input.pathIncludes&&!item.path.includes(input.pathIncludes)) continue;
      const lines = read(item.absolute).split(/\r?\n/);
      for (let index=0;index<lines.length;index++) {
        regex.lastIndex=0;
        if (regex.test(lines[index])) {
          matches.push({file:item.path,line:index+1,snippet:lines.slice(Math.max(0,index-(input.context||1)),index+(input.context||1)+1).join("\n")});
          if (matches.length>=(input.maxResults||200)) return {matches,truncated:true};
        }
      }
    }
    return {matches,truncated:false};
  }
  async function diffExplainer(input = {}) {
  ...
  function logSeverity(line) { return /fatal|panic|uncaught|segfault/i.test(line)?"critical":/error|failed|exception/i.test(line)?"high":/warn|deprecated/i.test(line)?"medium":"info"; }
  async function logAnalyzer(input = {}) {
    const lines = input.lines || String(input.text||"").split(/\r?\n/), clusters = new Map();
    for (const raw of lines) {
      if (!raw.trim()) continue;
      const timestamp = raw.match(/\d{4}-\d{2}-\d{2}[T ][\d:.+\-Z]+/)?.[0]||null;
      const signature = raw.replace(/\d{4}-\d{2}-\d{2}[T ][\d:.+\-Z]+/g,"<TS>").replace(/\bpid[=: ]?\d+/gi,"pid=<N>").replace(/\b[0-9a-f]{8}-[0-9a-f-]{27,}\b/gi,"<GUID>").replace(/0x[0-9a-f]+/gi,"<HEX>").replace(/:\d{2,5}\b/g,":<PORT>").replace(/([A-Za-z]:)?[\\/](?:[^\s:]+[\\/])+([^\\/\s:]+)/g,"<PATH>/$2").slice(0,300);
      const current = clusters.get(signature)||{signature,count:0,firstSeen:timestamp,lastSeen:timestamp,severity:logSeverity(raw),samples:[]};
      current.count+=1;current.lastSeen=timestamp||current.lastSeen;if(current.samples.length<3)current.samples.push(redact(raw).text);clusters.set(signature,current);
    }
    return {clusters:Array.from(clusters.values()).sort((a,b)=>b.count-a.count)};
  }
  async function apiContractReview(input = {}) {
    const issues=[];
    function inspect(schema,prefix,seen){if(!schema||typeof schema!=="object"||seen.has(schema))return;seen.add(schema);if(schema.type==="object"){for(const key of schema.required||[])if(!schema.properties?.[key])issues.push({severity:"high",path:prefix,code:"REQUIRED_MISSING",key});if(schema.additionalProperties===undefined)issues.push({severity:"low",path:prefix,code:"ADDITIONAL_UNSPECIFIED"});for(const[key,value]of Object.entries(schema.properties||{}))inspect(value,prefix+"."+key,seen);}if(schema.enum&&!schema.enum.length)issues.push({severity:"medium",path:prefix,code:"EMPTY_ENUM"});}
    inspect(input.schema,"$",new Set());
    for(const[route,item]of Object.entries(input.openapi?.paths||{}))for(const[method,operation]of Object.entries(item||{}))if(["get","post","put","patch","delete"].includes(method)&&!operation.responses)issues.push({severity:"high",path:route+":"+method,code:"RESPONSES_MISSING"});
    if(input.before&&input.after)for(const key of input.before.required||[])if(!(input.after.required||[]).includes(key))issues.push({severity:"high",code:"BREAKING_REQUIRED_REMOVED",key});
    if(input.ipc&&!input.ipc.requestId)issues.push({severity:"medium",code:"IPC_REQUEST_ID_MISSING"});
    return {issues};
  }
  ...
  async function workspaceDoctor(input = {}) { const root=input.root||process.cwd(),checks=[],exists=(name)=>fs.existsSync(path.join(root,name));let pkg={};try{pkg=JSON.parse(fs.readFileSync(path.join(root,"package.json"),"utf8"));}catch{}const projectFiles=fs.existsSync(root)?scan(root,{maxFiles:3000}):[],html=projectFiles.filter((item)=>item.ext===".html"&&!item.binary).map((item)=>read(item.absolute)).join("\n"),allText=projectFiles.filter((item)=>!item.binary&&item.size<500_000).map((item)=>read(item.absolute)).join("\n");checks.push({id:"node",ok:Number(process.versions.node.split(".")[0])>=20,value:process.versions.node},{id:"package",ok:exists("package.json")});const locks=["package-lock.json","pnpm-lock.yaml","yarn.lock","bun.lock","bun.lockb"].filter(exists);checks.push({id:"single-lock",ok:locks.length===1,value:locks},{id:"electron",ok:Boolean(pkg.dependencies?.electron||pkg.devDependencies?.electron)},{id:"builder",ok:Boolean(pkg.devDependencies?.["electron-builder"]||pkg.dependencies?.["electron-builder"])},{id:"test-script",ok:Boolean(pkg.scripts?.test)},{id:"csp",ok:/Content-Security-Policy/i.test(html)},{id:"preload",ok:projectFiles.some((item)=>/preload/i.test(item.path))},{id:"mojibake",ok:!/[ÃÄÅ�]/.test(allText)},{id:"windows-build",ok:Boolean(pkg.scripts?.dist||pkg.scripts?.build)});const env=["OPENAI_API_KEY","GEMINI_API_KEY","GOOGLE_API_KEY","ANTHROPIC_API_KEY","DEEPSEEK_API_KEY","KIMI_API_KEY","MOONSHOT_API_KEY","LOCAL_LLM_BASE_URL"].map((name)=>({name,set:Boolean(process.env[name])}));return{checks,env,summary:{passed:checks.filter((item)=>item.ok).length,total:checks.length}}; }
  async function architectureGraph(input = {}) { const nodes=[],edges=[];for(const item of safeScan(input)){if(!/[.](?:js|ts|tsx|jsx|py|cs|java)$/.test(item.path))continue;nodes.push({id:item.path,size:item.size});for(const target of moduleImports(read(item.absolute),item.ext))edges.push({from:item.path,to:target});}const inbound={},outbound={},adjacent={};for(const edge of edges){inbound[edge.to]=(inbound[edge.to]||0)+1;outbound[edge.from]=(outbound[edge.from]||0)+1;(adjacent[edge.from]||=[]).push(edge.to);}const cycles=[];function walk(node,stack){if(stack.includes(node)){cycles.push(stack.slice(stack.indexOf(node)).concat(node));return;}if(stack.length>20)return;for(const target of adjacent[node]||[])if(adjacent[target])walk(target,stack.concat(node));}for(const node of nodes.slice(0,1000))walk(node.id,[]);return{nodes,edges,cycles:Array.from(new Set(cycles.map(JSON.stringify))).map(JSON.parse).slice(0,100),highFanIn:Object.entries(inbound).sort((a,b)=>b[1]-a[1]).slice(0,20),highFanOut:Object.entries(outbound).sort((a,b)=>b[1]-a[1]).slice(0,20)}; }
  async function deadCodeDetector(input = {}) { const root=input.root||process.cwd();if(!fs.existsSync(root))return{candidates:[],warning:"Workspace yok"};const files=scan(root,{maxFiles:10000}).filter((item)=>!item.binary&&/[.](?:js|ts|tsx|jsx)$/.test(item.path)),used=new Set(),provided=new Map(),entrypoints=new Set(input.entrypoints||["index.js","main.js","src/index.js","src/main.js"]);for(const item of files){const source=read(item.absolute);provided.set(item.path,Array.from(source.matchAll(/(?:export\s+(?:function|class|const)\s+|exports\.|module\.exports\.)(\w+)/g)).map((match)=>match[1]));for(const target of moduleImports(source,item.ext))used.add(target);}const candidates=[];for(const[file,symbols]of provided)for(const symbol of symbols)if(!entrypoints.has(file)&&!Array.from(used).some((target)=>target.includes(path.basename(file,path.extname(file))))candidates.push({file,symbol,confidence:read(path.join(root,file)).includes("import(")?0.35:0.72,reason:"import graph'ta erişilemiyor"});return{candidates,warning:"Dinamik import ve reflection nedeniyle otomatik silme yapılmaz."}; }
  function normalizedTokens(source){return source.replace(/\/\*[\s\S]*?\*\/|\/\/.*$/gm,"").replace(/["'][^"']*["']/g,"STR").split(/[^A-Za-z0-9_]+/).filter(Boolean);}
  async function duplicateCodeFinder(input = {}) { if(!input.root)return{matches:[]};const minimum=input.minLines||6,threshold=input.threshold||0.8,blocks=[];for(const item of safeScan(input,{maxFiles:5000}).filter((entry)=>entry.size<1_000_000)){const lines=read(item.absolute).split(/\r?\n/);for(let index=0;index+minimum<=lines.length;index+=Math.max(1,Math.floor(minimum/2))){const tokens=normalizedTokens(lines.slice(index,index+minimum).join("\n"));if(tokens.length<4)continue;blocks.push({file:item.path,line:index+1,set:new Set(tokens),hash:crypto.createHash("sha1").update(tokens.join(" ")).digest("hex")});}}const matches=[];for(let left=0;left<blocks.length;left++)for(let right=left+1;right<blocks.length&&right<left+500;right++){if(blocks[left].file===blocks[right].file)continue;const intersection=Array.from(blocks[left].set).filter((token)=>blocks[right].set.has(token)).length,similarity=blocks[left].hash===blocks[right].hash?1:intersection/Math.max(blocks[left].set.size,blocks[right].set.size);if(similarity>=threshold)matches.push({a:{file:blocks[left].file,line:blocks[left].line},b:{file:blocks[right].file,line:blocks[right].line},similarity});}return{matches:matches.sort((a,b)=>b.similarity-a.similarity).slice(0,input.maxResults||200)}; }
  ...
  module.exports = {
    "repository-map":repositoryMap,"context-builder":contextBuilder,"code-review":codeReview,"test-generator":testGenerator,"bug-fix-planner":bugFixPlanner,
    "dependency-auditor":dependencyAuditor,"documentation-generator":documentationGenerator,"changelog-builder":changelogBuilder,"commit-message-assistant":commitMessageAssistant,"prompt-library":promptLibrary,
    "file-summarizer":fileSummarizer,"local-search":localSearch,"diff-explainer":diffExplainer,"log-analyzer":logAnalyzer,"api-contract-review":apiContractReview,"secret-scanner":secretScanner,
    "license-scanner":licenseScanner,"performance-hints":performanceHints,"refactor-planner":refactorPlanner,"workspace-doctor":workspaceDoctor,"architecture-graph":architectureGraph,"dead-code-detector":deadCodeDetector,
    "duplicate-code-finder":duplicateCodeFinder,"migration-planner":migrationPlanner,"ipc-contract-auditor":ipcContractAuditor,"electron-security-auditor":electronSecurityAuditor,"build-log-diagnoser":buildLogDiagnoser,
    "encoding-mojibake-doctor":encodingMojibakeDoctor,"config-drift-detector":configDriftDetector,"environment-key-doctor":environmentKeyDoctor,"git-risk-analyzer":gitRiskAnalyzer,"release-readiness":releaseReadiness,
    "test-gap-analyzer":testGapAnalyzer,"flaky-test-analyzer":flakyTestAnalyzer,"error-handling-auditor":errorHandlingAuditor,"accessibility-ui-auditor":accessibilityUiAuditor,"localization-integrity":localizationIntegrity,
    "workspace-indexer":workspaceIndexer,"dependency-upgrade-planner":dependencyUpgradePlanner,"patch-safety-validator":patchSafetyValidator
  };
}

function algorithmFiles() {
  const runtimeSource = "(" + algorithmsRuntime.toString() + ")();\n";
  const categories = {
    repository:["repository-map","context-builder","file-summarizer","local-search","workspace-indexer"],
    quality:["code-review","test-generator","bug-fix-planner","performance-hints","error-handling-auditor","accessibility-ui-auditor"],
    architecture:["architecture-graph","dead-code-detector","duplicate-code-finder","ipc-contract-auditor","electron-security-auditor"],
    release:["dependency-auditor","changelog-builder","migration-planner","git-risk-analyzer","release-readiness","patch-safety-validator"]
  };
  return {
    "src/shared/plugin-algorithms/index.js": runtimeSource,
    "src/shared/plugin-algorithms/repository.js": "\"use strict\";\nmodule.exports=" + JSON.stringify(categories.repository,null,2) + ";\n",
    "src/shared/plugin-algorithms/quality.js": "\"use strict\";\nmodule.exports=" + JSON.stringify(categories.quality,null,2) + ";\n",
    "src/shared/plugin-algorithms/architecture.js": "\"use strict\";\nmodule.exports=" + JSON.stringify(categories.architecture,null,2) + ";\n",
    "src/shared/plugin-algorithms/release.js": "\"use strict\";\nmodule.exports=" + JSON.stringify(categories.release,null,2) + ";\n"
  };
}
